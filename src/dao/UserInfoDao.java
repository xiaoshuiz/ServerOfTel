...
public class UserInfoDao {
	...
	public boolean login(String userName, String password) throws Exception {
		// 第一步：加载驱动（驱动jar包必须加入classpath中）
				...
				// 第四步：执行操作（增删改查）
				String sql = "SELECT * FROM datebaseofshui.user where username = '"+userName+"' and userpsd = '"+password+"';";
	...
	public boolean register(String userName, String password,String nickname)throws Exception{
		// 第一步：加载驱动（驱动jar包必须加入classpath中）
				Class.forName("com.mysql.jdbc.Driver");
				// 第二步：建立连接（根据实际情况替换数据库的主机地址、端口号、数据库明、登录名、密码）
				Connection conn = DriverManager
				...
				Statement stmt = conn.createStatement();// 只读的结果集
				// 第四步：执行操作（增删改查）
				String sql = "insert into info value(null,'" + userName + "','"
						+ password + "','" + nickname + "')";
				int rs = 1;
